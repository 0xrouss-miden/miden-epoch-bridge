import { epochExecutionError } from "@/lib/epoch-error";
import { EVM_TO_MIDEN_EXTRA_TYPESTRING, MIDEN_TO_EVM_EXTRA_TYPESTRING } from "@epoch-protocol/epoch-intents-sdk";
...
export async function getEVMToMidenQuote(
  ...
  return { taskTypeString, intentData, quoteResult, params: quoteParams, quotedAt: Date.now() };
}
...
export async function buildEVMToMidenIntent(
  ...
  try {
    ...
    const submitted = (err as { epochSubmission?: {
      stage: string; nonce: string; depositResult?: { success?: boolean; transactionHash?: string };
      intentRequestData: { compact: { sponsor: string; nonce: string; mandate: Record<string, unknown> } };
    } } | null)?.epochSubmission;
    if (submitted?.stage === 'allocation-response-unknown' && submitted.nonce === submitted.intentRequestData.compact.nonce &&
        submitted.intentRequestData.compact.sponsor.toLowerCase() === params.evmSourceAddress.toLowerCase()) {
      return {
        taskTypeString,
        intentData: submitted.intentRequestData.compact.mandate,
        intentNonce: submitted.nonce,
        depositChainId: params.sourceChainId,
        solveResult: { nonce: submitted.nonce, depositResult: submitted.depositResult },
        submissionWarning: epochExecutionError(err),
      };
    }
