...
beforeEach(() => {
  for (const key of ['NEXT_PUBLIC_MIDEN_NETWORK', 'NEXT_PUBLIC_MIDEN_RPC_URL', 'NEXT_PUBLIC_MIDEN_PROVER', 'NEXT_PUBLIC_MIDENSCAN_URL', 'NEXT_PUBLIC_MIDEN_USDC_FAUCET_ID']) {
...
describe('bridge network configuration', () => {
  it('defaults RPC, prover, and explorer to testnet', async () => {
    ...
    expect(config.MIDEN_USDC_FAUCET_ID).toBe("0x537c15a622074e91188aa894456c52");
  ...
  it('uses explicitly selected devnet and explorer override', async () => {
    ...
    vi.stubEnv('NEXT_PUBLIC_MIDENSCAN_URL', 'https://example.com/');
    vi.stubEnv('NEXT_PUBLIC_MIDEN_USDC_FAUCET_ID', '0xapproved');
    const config = await import('../../config');
    const explorers = await import('../explorers');
