...
export function IntentStatus({ result, error, flowStatus, isPolling }: Props) {
  ...
  return (
    <div className="ui-card space-y-3">
      {result.submissionWarning && (
        <div role="status" className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
          <h2 className="mb-2 font-semibold">{evmCompleted ? 'Delivery confirmed by Epoch' : 'Checking delivery'}</h2>
          <p>{evmCompleted ? 'Intent tracking confirms execution on the destination chain despite the initial HTTP error. Consume the delivered note to add the tokens to your balance.' : 'Epoch returned an error in response to the request. This does not confirm that the transfer failed. We are tracking the same intent without repeating the deposit.'}</p>
          <p className="mt-2 break-all">Intent: {result.intentNonce}</p>
          <details className="mt-2"><summary>Original Epoch response</summary><p>{result.submissionWarning}</p></details>
        </div>
      )}
      {stillWaiting && (
